...
function convertToColorSequence(colorTable)
    local keypoints = {}

    for i = 1, #colorTable, 4 do
        local time = colorTable[i] -- ค่า Time Position
        local r = colorTable[i+1] * 255 -- ค่า Red (0-255)
        local g = colorTable[i+2] * 255 -- ค่า Green (0-255)
        local b = colorTable[i+3] * 255 -- ค่า Blue (0-255)
...
function colorSequenceToString(colorSequence)
    local result = {}

    for _, keypoint in ipairs(colorSequence.Keypoints) do
        local time = keypoint.Time
        local color = keypoint.Value
        table.insert(result, string.format("%.6f %.6f %.6f %.6f", time, color.R, color.G, color.B))
    end

    return table.concat(result, " ")
end

function stringToColorSequence(str)
    local data = {}
    
    for time, r, g, b in str:gmatch("([%d%.]+) ([%d%.]+) ([%d%.]+) ([%d%.]+)") do
        table.insert(data, tonumber(time))
        table.insert(data, tonumber(r))
        table.insert(data, tonumber(g))
        table.insert(data, tonumber(b))
    end

    return convertToColorSequence(data)
end
...
function Library.Create(name,parent,Title,SaveManager,default)
    ...
    local function CreateList(itemName,imageId,Gradient)
        ...
        if Gradient then 
            local GradientColor = stringToColorSequence(Gradient)
            local newGradient = Instance.new("UIGradient")
            newGradient.Color = GradientColor
            newGradient.Parent = selectedItemImage
        end;

        warn('insert')
    end;

    function shopUI.new(itemName, description, price,imageId,Gradient)
        ...
        buyButton.Size = itemFrame.Size
        --UDim2.new(0, 80, 0, 30)
        buyButton.Position = itemFrame.Position
        --UDim2.new(1, -90, 0, 35)
